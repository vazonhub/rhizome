...
class RateLimiter:
    ...
    def __init__(
        ...
        # Запросы по узлам
        self.node_requests: Dict[bytes, deque] = defaultdict(lambda: deque(maxlen=per_node_limit * 2))
    ...
    def check_rate_limit(self, node_id: Optional[bytes] = None) -> bool:
        ...
        if node_id:
            node_recent = sum(
                1 for timestamp in self.node_requests[node_id]
                if current_time - timestamp < self.window_seconds
            )
    ...
    def _cleanup_old_requests(self, current_time: float):
        ...
        while self.request_history and (current_time - self.request_history[0]) > self.window_seconds:
