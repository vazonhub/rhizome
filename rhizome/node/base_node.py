...
from rhizome.utils.crypto import generate_node_id, save_node_id, load_node_id
from rhizome.exceptions import InvalidNodeTypeError
from rhizome.logger import setup_logging, get_logger
    ...
    async def bootstrap(self):
        ...
        for bootstrap_addr in bootstrap_nodes:
            try:
                ...
                bootstrap_node = Node(
                    node_id=NodeID(id=b'\x00' * 20),  # Временный ID
                    address=host,
                    port=port
                )
    ...
    async def _update_popularity_rankings(self):
        ...
        if popular_items:
            await self.replicator.replicate_popular_items(
                popular_items,
                popularity_threshold=self.config.popularity.popularity_threshold
            )
