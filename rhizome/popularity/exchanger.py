...
class PopularityExchanger:
    ...
    async def exchange_top_items(
        self,
        local_metrics: Dict[bytes, PopularityMetrics],
        neighbor_nodes: List,
        top_n: int = 100
            ...
            try:
                ...
                if key in self.metrics_collector.metrics:
                    ...
                    self.metrics_collector.metrics[key].update_replication(
                        max(self.metrics_collector.metrics[key].replication_count, received_replication)
                    )
            except Exception as e:
                self.logger.warning("Error processing received item", error=str(e))
    ...
    async def aggregate_global_ranking(
        self,
        local_rankings: List[RankedItem],
        ...
        for key, scores in all_rankings.items():
            if scores:
                ...
                if metrics:
                    consensus_ranking.append(RankedItem(
                        key=key,
                        score=median_score,
                        metrics=metrics
                    ))
