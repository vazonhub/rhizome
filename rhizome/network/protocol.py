...
from rhizome.network.transport import UDPTransport, Message
    ...
    async def _handle_request(
        ...
        if msg_type == self.MSG_PING:
            ...
            response = self._pack_message(
                self.MSG_PONG,
                msg_id,
                {"node_id": self.node_id.id, "address": self.local_address}
            )
            ...
            if "items" in payload and self.popularity_exchanger:
                try:
                    ...
                    response = self._pack_message(
                        self.MSG_POPULARITY_EXCHANGE_RESPONSE,
                        msg_id,
                    ...
                    self.logger.error("Error handling popularity exchange", error=str(e), exc_info=True)
            ...
            if self.popularity_exchanger:
                try:
                    ...
                    response = self._pack_message(
                        self.MSG_GLOBAL_RANKING_RESPONSE,
                        msg_id,
                        {"ranking": global_ranking}
                    )
                    await self.transport.send(response, address)
                except Exception as e:
                    self.logger.error("Error handling global ranking request", error=str(e), exc_info=True)
