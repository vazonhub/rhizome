...
class Replicator:
    ...
    async def replicate_popular_items(
        ...
        popular_items = [
            ...
            if item.score >= popularity_threshold
        ]
    ...
    async def emergency_replication(
        self,
        key: bytes,
        value: bytes
    ) -> bool:
        ...
        try:
            ...
            self.logger.error(
                "Error in emergency replication",
                key=key.hex()[:16],
                error=str(e),
                exc_info=True
            )
